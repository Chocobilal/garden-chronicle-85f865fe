...
export const GARDEN = {
  ...
  tagline: "Founded in the blessed month of the Prophet's birth ﷺ — a garden of gratitude and growth.",
  location: "No.38/4, 4 Mile Post, Govi Gammanaya, Keliyapura, Hambantota 82000, Sri Lanka",
...
export const getAdjacentTreeIds = (id: string) => {
  ...
  const prev = ALL_TREE_IDS[(i - 1 + ALL_TREE_IDS.length) % ALL_TREE_IDS.length];
...
export const healthColor = (s: HealthStatus) => {
  switch (s) {
    ...
    case "Monitor": return "bg-health-attention";
    case "Attention": return "bg-health-critical";
  }
};
...
export const healthDot = (s: HealthStatus) => {
  switch (s) {
    ...
    case "Monitor": return "hsl(var(--health-attention))";
    case "Attention": return "hsl(var(--health-critical))";
  }
};
...
export const healthBadge = (s: HealthStatus) => {
  switch (s) {
    ...
    case "Monitor": return "bg-health-attention/15 text-health-attention border-health-attention/40";
    case "Attention": return "bg-health-critical/15 text-health-critical border-health-critical/40";
  }
};
...
export const healthLabel = (s: HealthStatus) => {
  switch (s) {
    ...
    case "Monitor": return "⚠️ Needs Attention";
    case "Attention": return "🚨 Critical";
  }
};
